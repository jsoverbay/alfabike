...
#        self.image.convert()
	...
	self.image = self.imageMaster	
        ...
        if up:
            ...
            if self.onGround: self.yvel -= 6   #uncomment to allow jumping
		#if not in lane 1 allow moving up
		#self.rect = self.rect.move(0,-32)
	#		pass #comment to allow jumping
        if down:
		#if not in lane 4 allow moving down
            	#self.rect = self.rect.move(0,32)
		pass
	if rotleft:
		self.dir += 45
